...
b2Settings.b2_velocityThreshold =
	(1.0 * b2Settings.b2_lengthUnitsPerMeter) / b2Settings.b2_timeUnitsPerSecond;
...
b2Settings.b2_linearSleepTolerance =
	(0.01 * b2Settings.b2_lengthUnitsPerMeter) / b2Settings.b2_timeUnitsPerSecond;
b2Settings.b2_angularSleepTolerance =
	2.0 / 180.0 / b2Settings.b2_timeUnitsPerSecond;
...
b2Math.b2MulTMM = function (A, B) {
	var c1 = new b2Vec2(
		b2Math.b2Dot(A.col1, B.col1),
		b2Math.b2Dot(A.col2, B.col1)
	);
	var c2 = new b2Vec2(
		b2Math.b2Dot(A.col1, B.col2),
		b2Math.b2Dot(A.col2, B.col2)
	);
...
b2PairManager.prototype = {
	...
	Commit: function () {
		...
		for (i = 0; i < this.m_pairBufferCount; ++i) {
			var pair = this.Find(
				this.m_pairBuffer[i].proxyId1,
				this.m_pairBuffer[i].proxyId2
			);
			pair.ClearBuffered();
			...
			if (pair.IsRemoved()) {
				...
				if (pair.IsFinal() == false) {
					pair.userData = this.m_callback.PairAdded(
						proxy1.userData,
						proxy2.userData
					);
					pair.SetFinal();
	...
	FindHash: function (proxyId1, proxyId2, hash) {
		...
		while (
			index != b2Pair.b2_nullPair &&
			b2PairManager.Equals(this.m_pairs[index], proxyId1, proxyId2) == false
		) {
...
b2BroadPhase.prototype = {
	...
	GetProxy: function (proxyId) {
		if (
			proxyId == b2Pair.b2_nullProxy ||
			this.m_proxyPool[proxyId].IsValid() == false
		) {
			return null;
		}
	...
	CreateProxy: function (aabb, userData) {
		...
		for (var axis = 0; axis < 2; ++axis) {
			...
			bounds[lowerIndex].stabbingCount =
				lowerIndex == 0 ? 0 : bounds[lowerIndex - 1].stabbingCount;
	...
	MoveProxy: function (proxyId, aabb) {
		...
		for (axis = 0; axis < 2; ++axis) {
			oldValues.lowerValues[axis] =
				this.m_bounds[axis][proxy.lowerBounds[axis]].value;
			oldValues.upperValues[axis] =
				this.m_bounds[axis][proxy.upperBounds[axis]].value;
		}
		for (axis = 0; axis < 2; ++axis) {
			var bounds = this.m_bounds[axis];
			var lowerIndex = proxy.lowerBounds[axis];
	...
	ComputeBounds: function (lowerValues, upperValues, aabb) {
		...
		lowerValues[0] =
			(this.m_quantizationFactor.x *
				(minVertexX - this.m_worldAABB.minVertex.x)) &
			(b2Settings.USHRT_MAX - 1);
		upperValues[0] =
			((this.m_quantizationFactor.x *
				(maxVertexX - this.m_worldAABB.minVertex.x)) &
				0x0000ffff) |
			1;
		lowerValues[1] =
			(this.m_quantizationFactor.y *
				(minVertexY - this.m_worldAABB.minVertex.y)) &
			(b2Settings.USHRT_MAX - 1);
		upperValues[1] =
			((this.m_quantizationFactor.y *
				(maxVertexY - this.m_worldAABB.minVertex.y)) &
				0x0000ffff) |
			1;
	},
	TestOverlapValidate: function (p1, p2) {
		for (var axis = 0; axis < 2; ++axis) {
			var bounds = this.m_bounds[axis];
			if (
				bounds[p1.lowerBounds[axis]].value > bounds[p2.upperBounds[axis]].value
			)
				return false;
			if (
				bounds[p1.upperBounds[axis]].value < bounds[p2.lowerBounds[axis]].value
			)
				return false;
		}
		return true;
...
b2Collision.EdgeSeparation = function (poly1, edge1, poly2) {
	...
	var v1X =
		poly1.m_position.x +
		(tMat.col1.x * vert1s[edge1].x + tMat.col2.x * vert1s[edge1].y);
	var v1Y =
		poly1.m_position.y +
		(tMat.col1.y * vert1s[edge1].x + tMat.col2.y * vert1s[edge1].y);
	tMat = poly2.m_R;
	var v2X =
		poly2.m_position.x +
		(tMat.col1.x * vert2s[vertexIndex2].x +
			tMat.col2.x * vert2s[vertexIndex2].y);
	var v2Y =
		poly2.m_position.y +
		(tMat.col1.y * vert2s[vertexIndex2].x +
			tMat.col2.y * vert2s[vertexIndex2].y);
	v2X -= v1X;
	v2Y -= v1Y;
...
b2Collision.FindIncidentEdge = function (c, poly1, edge1, poly2) {
	...
	var invLength =
		1.0 /
		Math.sqrt(
			normal1Local1X * normal1Local1X + normal1Local1Y * normal1Local1Y
		);
	...
	for (var i = 0; i < count2; ++i) {
		...
		invLength =
			1.0 /
			Math.sqrt(
				normal2Local2X * normal2Local2X + normal2Local2Y * normal2Local2Y
			);
...
b2Collision.b2CollidePoly = function (manifold, polyA, polyB, conservative) {
	...
	var invLength =
		1.0 / Math.sqrt(sideNormalX * sideNormalX + sideNormalY * sideNormalY);
	...
	for (var i = 0; i < b2Settings.b2_maxManifoldPoints; ++i) {
		...
		var separation =
			frontNormalX * tVec.x + frontNormalY * tVec.y - frontOffset;
...
b2Collision.b2CollideCircle = function (
	manifold,
	circle1,
	circle2,
	conservative
	...
	tPoint.position.x =
		circle2.m_position.x - circle2.m_radius * manifold.normal.x;
	tPoint.position.y =
		circle2.m_position.y - circle2.m_radius * manifold.normal.y;
};
b2Collision.b2CollidePolyAndCircle = function (
	manifold,
	poly,
	circle,
	conservative
	...
	for (var i = 0; i < poly.m_vertexCount; ++i) {
		var s =
			poly.m_normals[i].x * (xLocalX - poly.m_vertices[i].x) +
			poly.m_normals[i].y * (xLocalY - poly.m_vertices[i].y);
		if (s > radius) {
			return;
		}
		if (s > separation) {
	...
	if (length < Number.MIN_VALUE) {
		...
		manifold.pointCount = 1;
		manifold.normal.Set(
			tMat.col1.x * dX + tMat.col2.x * dY,
			tMat.col1.y * dX + tMat.col2.y * dY
		);
		tPoint = manifold.points[0];
	...
	var u =
		(xLocalX - poly.m_vertices[vertIndex1].x) * eX +
		(xLocalY - poly.m_vertices[vertIndex1].y) * eY;
	tPoint = manifold.points[0];
	tPoint.id.features.incidentEdge = b2Collision.b2_nullFeature;
	tPoint.id.features.incidentVertex = b2Collision.b2_nullFeature;
	tPoint.id.features.referenceFace = b2Collision.b2_nullFeature;
...
Features.prototype = {
	set_referenceFace: function (value) {
		...
		this._m_id._key =
			(this._m_id._key & 0xffffff00) | (this._referenceFace & 0x000000ff);
	},
			...
			case b2Shape.e_circleShape:
				{
					var circle = this;
					massData.mass =
						this.density * b2Settings.b2_pi * circle.radius * circle.radius;
					massData.center.Set(0.0, 0.0);
			...
			case b2Shape.e_boxShape:
				{
					...
					massData.center.Set(0.0, 0.0);
					massData.I =
						(massData.mass / 3.0) * b2Math.b2Dot(box.extents, box.extents);
				}
				break;
			case b2Shape.e_polyShape:
				{
...
Object.extend(b2CircleShape.prototype, {
	...
	initialize: function (def, body, localCenter) {
		...
		this.m_localPosition.Set(
			def.localPosition.x - localCenter.x,
			def.localPosition.y - localCenter.y
		);
	...
	Synchronize: function (position1, R1, position2, R2) {
		this.m_R.SetM(R2);
		this.m_position.x =
			R2.col1.x * this.m_localPosition.x +
			R2.col2.x * this.m_localPosition.y +
			position2.x;
		this.m_position.y =
			R2.col1.y * this.m_localPosition.x +
			R2.col2.y * this.m_localPosition.y +
			position2.y;
		if (this.m_proxyId == b2Pair.b2_nullProxy) {
			return;
		}
		var p1X =
			position1.x +
			(R1.col1.x * this.m_localPosition.x + R1.col2.x * this.m_localPosition.y);
		var p1Y =
			position1.y +
			(R1.col1.y * this.m_localPosition.x + R1.col2.y * this.m_localPosition.y);
	...
	Support: function (dX, dY, out) {
		...
		dY /= len;
		out.Set(
			this.m_position.x + this.m_radius * dX,
...
Object.extend(b2PolyShape.prototype, {
	...
	initialize: function (def, body, newOrigin) {
		...
		if (def.type == b2Shape.e_boxShape) {
			...
			this.m_localCentroid.x =
				def.localPosition.x +
				(localR.col1.x * centroidX + localR.col2.x * centroidY) -
				newOrigin.x;
			this.m_localCentroid.y =
				def.localPosition.y +
				(localR.col1.y * centroidX + localR.col2.y * centroidY) -
				newOrigin.y;
			for (i = 0; i < this.m_vertexCount; ++i) {
				...
				this.m_coreVertices[i].x =
					this.m_vertices[i].x - 2.0 * b2Settings.b2_linearSlop * uX;
				this.m_coreVertices[i].y =
					this.m_vertices[i].y - 2.0 * b2Settings.b2_linearSlop * uY;
			}
		}
		...
		this.m_localOBB.center.Set(
			(minVertexX + maxVertexX) * 0.5,
			(minVertexY + maxVertexY) * 0.5
		);
		this.m_localOBB.extents.Set(
			(maxVertexX - minVertexX) * 0.5,
			(maxVertexY - minVertexY) * 0.5
		);
		var i1 = 0;
		var i2 = 0;
		for (i = 0; i < this.m_vertexCount; ++i) {
	...
	Synchronize: function (position1, R1, position2, R2) {
		...
		this.m_position.x =
			this.m_body.m_position.x +
			(R2.col1.x * this.m_localCentroid.x + R2.col2.x * this.m_localCentroid.y);
		this.m_position.y =
			this.m_body.m_position.y +
			(R2.col1.y * this.m_localCentroid.x + R2.col2.y * this.m_localCentroid.y);
		if (this.m_proxyId == b2Pair.b2_nullProxy) {
			return;
		}
		var hX;
	...
	Support: function (dX, dY, out) {
		...
		var bestIndex = 0;
		var bestValue =
			this.m_coreVertices[0].x * dLocalX + this.m_coreVertices[0].y * dLocalY;
		for (var i = 1; i < this.m_vertexCount; ++i) {
			var value =
				this.m_coreVertices[i].x * dLocalX + this.m_coreVertices[i].y * dLocalY;
			if (value > bestValue) {
				bestIndex = i;
				bestValue = value;
			}
		...
		out.Set(
			this.m_position.x +
				(this.m_R.col1.x * this.m_coreVertices[bestIndex].x +
					this.m_R.col2.x * this.m_coreVertices[bestIndex].y),
			this.m_position.y +
				(this.m_R.col1.y * this.m_coreVertices[bestIndex].x +
					this.m_R.col2.y * this.m_coreVertices[bestIndex].y)
		);
	},
...
b2Body.prototype = {
	...
	ApplyForce: function (force, point) {
		if (this.IsSleeping() == false) {
			...
			this.m_torque += b2Math.b2CrossVV(
				b2Math.SubtractVV(point, this.m_position),
				force
			);
		}
	},
	...
	ApplyImpulse: function (impulse, point) {
		if (this.IsSleeping() == false) {
			...
			this.m_angularVelocity +=
				this.m_invI *
				b2Math.b2CrossVV(b2Math.SubtractVV(point, this.m_position), impulse);
		}
	},
	GetMass: function () {
		return this.m_mass;
	...
	GetLocalPoint: function (worldPoint) {
		return b2Math.b2MulTMV(
			this.m_R,
			b2Math.SubtractVV(worldPoint, this.m_position)
		);
	},
	...
	initialize: function (bd, world) {
		...
		for (i = 0; i < b2Settings.b2_maxShapesPerBody; ++i) {
			...
			this.m_center.x +=
				massData.mass * (sd.localPosition.x + massData.center.x);
			this.m_center.y +=
				massData.mass * (sd.localPosition.y + massData.center.y);
			++this.m_shapeCount;
		}
		if (this.m_mass > 0.0) {
		...
		for (i = 0; i < this.m_shapeCount; ++i) {
			...
			massData = massDatas[i];
			this.m_I += massData.I;
			var r = b2Math.SubtractVV(
				b2Math.AddVV(sd.localPosition, massData.center),
				this.m_center
			);
		...
		this.m_linearVelocity = b2Math.AddVV(
			bd.linearVelocity,
			b2Math.b2CrossFV(bd.angularVelocity, this.m_center)
		);
...
b2CollisionFilter.prototype = {
	ShouldCollide: function (shape1, shape2) {
		if (
			shape1.m_groupIndex == shape2.m_groupIndex &&
			shape1.m_groupIndex != 0
		) {
			return shape1.m_groupIndex > 0;
		}
		var collide =
			(shape1.m_maskBits & shape2.m_categoryBits) != 0 &&
...
b2Island.prototype = {
	initialize: function (
		...
		for (i = 0; i < bodyCapacity; i++) this.m_bodies[i] = null;
		this.m_contacts = new Array(contactCapacity);
		for (i = 0; i < contactCapacity; i++) this.m_contacts[i] = null;
		this.m_joints = new Array(jointCapacity);
		for (i = 0; i < jointCapacity; i++) this.m_joints[i] = null;
	...
	Solve: function (step, gravity) {
		...
		for (i = 0; i < this.m_bodyCount; ++i) {
			...
			b.m_linearVelocity.Add(
				b2Math.MulFV(
					step.dt,
					b2Math.AddVV(gravity, b2Math.MulFV(b.m_invMass, b.m_force))
				)
			);
		...
		var contactSolver = new b2ContactSolver(
			this.m_contacts,
			this.m_contactCount,
			this.m_allocator
		);
	...
	UpdateSleep: function (dt) {
		...
		var angTolSqr =
			b2Settings.b2_angularSleepTolerance * b2Settings.b2_angularSleepTolerance;
		for (i = 0; i < this.m_bodyCount; ++i) {
			b = this.m_bodies[i];
			if (b.m_invMass == 0.0) {
				continue;
		...
		for (i = 0; i < contactCount; ++i) {
			...
			for (var j = 0; j < manifoldCount; ++j) {
				...
				for (var k = 0; k < c.pointCount; ++k) {
					...
					kNormal +=
						b1.m_invI * (r1Sqr - rn1 * rn1) + b2.m_invI * (r2Sqr - rn2 * rn2);
					...
					kTangent +=
						b1.m_invI * (r1Sqr - rt1 * rt1) + b2.m_invI * (r2Sqr - rt2 * rt2);
	...
	SolveVelocityConstraints: function () {
		...
		for (var i = 0; i < this.m_constraintCount; ++i) {
			...
			for (j = 0; j < tCount; ++j) {
				...
				dvX =
					b2_linearVelocity.x +
					-b2_angularVelocity * r2Y -
					b1_linearVelocity.x -
					-b1_angularVelocity * r1Y;
				dvY =
					b2_linearVelocity.y +
					b2_angularVelocity * r2X -
					b1_linearVelocity.y -
					b1_angularVelocity * r1X;
		...
		for (var i = 0; i < this.m_constraintCount; ++i) {
			...
			for (var j = 0; j < tCount; ++j) {
				...
				var C =
					beta *
					b2Math.b2Clamp(
						separation + b2Settings.b2_linearSlop,
...
b2Conservative.Conservative = function (shape1, shape2) {
	...
	var hit = true;
	for (var iter = 0; iter < maxIterations; ++iter) {
		var distance = b2Distance.Distance(
			b2Conservative.x1,
			b2Conservative.x2,
			shape1,
			shape2
...
Object.extend(b2ContactManager.prototype, {
	...
	PairAdded: function (proxyUserData1, proxyUserData2) {
		...
		if (
			...
			this.m_world.m_filter.ShouldCollide(shape1, shape2) == false
		) {
			return this.m_nullContact;
		}
		...
		var contact = b2Contact.Create(
			shape1,
			shape2,
			this.m_world.m_blockAllocator
		);
		if (contact == null) {
			return this.m_nullContact;
		} else {
...
b2World.prototype = {
	...
	CreateJoint: function (def) {
		...
		if (def.collideConnected == false) {
			var b =
				def.body1.m_shapeCount < def.body2.m_shapeCount ? def.body1 : def.body2;
	...
	Step: function (dt, iterations) {
		...
		for (var k = 0; k < this.m_bodyCount; k++) stack[k] = null;
...
Object.extend(b2DistanceJoint.prototype, {
	...
	PrepareVelocitySolver: function () {
		...
		tMat = this.m_body1.m_R;
		var r1X =
			tMat.col1.x * this.m_localAnchor1.x + tMat.col2.x * this.m_localAnchor1.y;
		var r1Y =
			tMat.col1.y * this.m_localAnchor1.x + tMat.col2.y * this.m_localAnchor1.y;
		tMat = this.m_body2.m_R;
		var r2X =
			tMat.col1.x * this.m_localAnchor2.x + tMat.col2.x * this.m_localAnchor2.y;
		var r2Y =
			tMat.col1.y * this.m_localAnchor2.x + tMat.col2.y * this.m_localAnchor2.y;
		this.m_u.x =
			this.m_body2.m_position.x + r2X - this.m_body1.m_position.x - r1X;
		this.m_u.y =
			this.m_body2.m_position.y + r2Y - this.m_body1.m_position.y - r1Y;
		...
		this.m_mass =
			...
			this.m_body1.m_invI * cr1u * cr1u +
			this.m_body2.m_invMass +
			this.m_body2.m_invI * cr2u * cr2u;
		...
		if (b2World.s_enableWarmStarting) {
			...
			this.m_body1.m_angularVelocity -=
				this.m_body1.m_invI * (r1X * PY - r1Y * PX);
			...
			this.m_body2.m_angularVelocity +=
				this.m_body2.m_invI * (r2X * PY - r2Y * PX);
		} else {
			this.m_impulse = 0.0;
		}
	},
	SolveVelocityConstraints: function (step) {
		...
		var r2Y =
			tMat.col1.y * this.m_localAnchor2.x + tMat.col2.y * this.m_localAnchor2.y;
		var v1X =
			this.m_body1.m_linearVelocity.x + -this.m_body1.m_angularVelocity * r1Y;
		var v1Y =
			this.m_body1.m_linearVelocity.y + this.m_body1.m_angularVelocity * r1X;
		var v2X =
			this.m_body2.m_linearVelocity.x + -this.m_body2.m_angularVelocity * r2Y;
		var v2Y =
			this.m_body2.m_linearVelocity.y + this.m_body2.m_angularVelocity * r2X;
...
Object.extend(b2GearJoint.prototype, {
	GetAnchor1: function () {
		...
		return new b2Vec2(
			this.m_body1.m_position.x +
				(tMat.col1.x * this.m_localAnchor1.x +
					tMat.col2.x * this.m_localAnchor1.y),
			this.m_body1.m_position.y +
				(tMat.col1.y * this.m_localAnchor1.x +
					tMat.col2.y * this.m_localAnchor1.y)
		);
	},
	GetAnchor2: function () {
		...
		return new b2Vec2(
			this.m_body2.m_position.x +
				(tMat.col1.x * this.m_localAnchor2.x +
					tMat.col2.x * this.m_localAnchor2.y),
			this.m_body2.m_position.y +
				(tMat.col1.y * this.m_localAnchor2.x +
					tMat.col2.y * this.m_localAnchor2.y)
		);
	},
	GetReactionForce: function (invTimeStep) {
		return new b2Vec2();
	...
	PrepareVelocitySolver: function () {
		...
		if (this.m_revolute2) {
			...
			K +=
				this.m_ratio * this.m_ratio * (b2.m_invMass + b2.m_invI * crug * crug);
		}
		this.m_mass = 1.0 / K;
...
Object.extend(b2MouseJoint.prototype, {
	...
	initialize: function (def) {
		...
		this.m_localAnchor.x =
			tX * this.m_body2.m_R.col1.x + tY * this.m_body2.m_R.col1.y;
		this.m_localAnchor.y =
			tX * this.m_body2.m_R.col2.x + tY * this.m_body2.m_R.col2.y;
	...
	PrepareVelocitySolver: function () {
		...
		tMat = b.m_R;
		var rX =
			tMat.col1.x * this.m_localAnchor.x + tMat.col2.x * this.m_localAnchor.y;
		var rY =
			tMat.col1.y * this.m_localAnchor.x + tMat.col2.y * this.m_localAnchor.y;
	...
	SolveVelocityConstraints: function (step) {
		...
		var tX =
			CdotX +
			this.m_beta * step.inv_dt * this.m_C.x +
			this.m_gamma * this.m_impulse.x;
		var tY =
			CdotY +
			this.m_beta * step.inv_dt * this.m_C.y +
...
Object.extend(b2PrismaticJoint.prototype, {
	...
	GetJointTranslation: function () {
		...
		tMat = b1.m_R;
		var ax1X =
			tMat.col1.x * this.m_localXAxis1.x + tMat.col2.x * this.m_localXAxis1.y;
		var ax1Y =
			tMat.col1.y * this.m_localXAxis1.x + tMat.col2.y * this.m_localXAxis1.y;
	...
	GetJointSpeed: function () {
		...
		var speed =
			dX * (-w1 * ax1Y) +
			dY * (w1 * ax1X) +
			(ax1X * (v2.x + -w2 * r2Y - v1.x - -w1 * r1Y) +
				ax1Y * (v2.y + w2 * r2X - v1.y - w1 * r1X));
		return speed;
	},
	...
	GetReactionForce: function (invTimeStep) {
		...
		var ay1X =
			tImp *
			(tMat.col1.x * this.m_localYAxis1.x + tMat.col2.x * this.m_localYAxis1.y);
		var ay1Y =
			tImp *
			(tMat.col1.y * this.m_localYAxis1.x + tMat.col2.y * this.m_localYAxis1.y);
	...
	initialize: function (def) {
		...
		this.m_localAnchor1.Set(
			tX * tMat.col1.x + tY * tMat.col1.y,
			tX * tMat.col2.x + tY * tMat.col2.y
		);
		tMat = this.m_body2.m_R;
	...
	SolveVelocityConstraints: function (step) {
		...
		var invI2 = b2.m_invI;
		var oldLimitImpulse;
		var linearCdot = this.m_linearJacobian.Compute(
			b1.m_linearVelocity,
			b1.m_angularVelocity,
			b2.m_linearVelocity,
			b2.m_angularVelocity
		...
		b1.m_linearVelocity.x +=
			invMass1 * linearImpulse * this.m_linearJacobian.linear1.x;
		b1.m_linearVelocity.y +=
			invMass1 * linearImpulse * this.m_linearJacobian.linear1.y;
		b1.m_angularVelocity +=
			invI1 * linearImpulse * this.m_linearJacobian.angular1;
		b2.m_linearVelocity.x +=
			invMass2 * linearImpulse * this.m_linearJacobian.linear2.x;
		b2.m_linearVelocity.y +=
			invMass2 * linearImpulse * this.m_linearJacobian.linear2.y;
		b2.m_angularVelocity +=
			invI2 * linearImpulse * this.m_linearJacobian.angular2;
		...
		if (this.m_enableMotor && this.m_limitState != b2Joint.e_equalLimits) {
			...
			this.m_motorImpulse = b2Math.b2Clamp(
				this.m_motorImpulse + motorImpulse,
				-step.dt * this.m_maxMotorForce,
				step.dt * this.m_maxMotorForce
			);
			motorImpulse = this.m_motorImpulse - oldMotorImpulse;
			b1.m_linearVelocity.x +=
				invMass1 * motorImpulse * this.m_motorJacobian.linear1.x;
			b1.m_linearVelocity.y +=
				invMass1 * motorImpulse * this.m_motorJacobian.linear1.y;
			b1.m_angularVelocity +=
				invI1 * motorImpulse * this.m_motorJacobian.angular1;
			b2.m_linearVelocity.x +=
				invMass2 * motorImpulse * this.m_motorJacobian.linear2.x;
			b2.m_linearVelocity.y +=
				invMass2 * motorImpulse * this.m_motorJacobian.linear2.y;
			b2.m_angularVelocity +=
				invI2 * motorImpulse * this.m_motorJacobian.angular2;
		}
		if (this.m_enableLimit && this.m_limitState != b2Joint.e_inactiveLimit) {
			var limitCdot = this.m_motorJacobian.Compute(
				b1.m_linearVelocity,
				b1.m_angularVelocity,
				b2.m_linearVelocity,
				b2.m_angularVelocity
			...
			if (this.m_limitState == b2Joint.e_equalLimits) {
				...
				this.m_limitImpulse = b2Math.b2Max(
					this.m_limitImpulse + limitImpulse,
					0.0
				);
				...
				oldLimitImpulse = this.m_limitImpulse;
				this.m_limitImpulse = b2Math.b2Min(
					this.m_limitImpulse + limitImpulse,
					0.0
				);
				limitImpulse = this.m_limitImpulse - oldLimitImpulse;
			...
			b1.m_linearVelocity.x +=
				invMass1 * limitImpulse * this.m_motorJacobian.linear1.x;
			b1.m_linearVelocity.y +=
				invMass1 * limitImpulse * this.m_motorJacobian.linear1.y;
			b1.m_angularVelocity +=
				invI1 * limitImpulse * this.m_motorJacobian.angular1;
			b2.m_linearVelocity.x +=
				invMass2 * limitImpulse * this.m_motorJacobian.linear2.x;
			b2.m_linearVelocity.y +=
				invMass2 * limitImpulse * this.m_motorJacobian.linear2.y;
			b2.m_angularVelocity +=
				invI2 * limitImpulse * this.m_motorJacobian.angular2;
		}
	},
	SolvePositionConstraints: function () {
		var limitC;
		...
		if (this.m_enableLimit && this.m_limitState != b2Joint.e_inactiveLimit) {
			...
			if (this.m_limitState == b2Joint.e_equalLimits) {
				...
				limitC = b2Math.b2Clamp(
					limitC + b2Settings.b2_linearSlop,
					-b2Settings.b2_maxLinearCorrection,
					0.0
				);
				limitImpulse = -this.m_motorMass * limitC;
				...
				this.m_limitPositionImpulse = b2Math.b2Max(
					this.m_limitPositionImpulse + limitImpulse,
					0.0
				);
	...
	SolvePositionConstraints: function () {
		...
		if (this.m_limitState1 == b2Joint.e_atUpperLimit) {
			...
			this.m_limitPositionImpulse1 = b2Math.b2Max(
				0.0,
				this.m_limitPositionImpulse1 + impulse
			);
		...
		if (this.m_limitState2 == b2Joint.e_atUpperLimit) {
			...
			this.m_limitPositionImpulse2 = b2Math.b2Max(
				0.0,
				this.m_limitPositionImpulse2 + impulse
			);
...
Object.extend(b2RevoluteJoint.prototype, {
	...
	PrepareVelocitySolver: function () {
		...
		if (b2World.s_enableWarmStarting) {
			...
			b1.m_angularVelocity -=
				invI1 *
				(r1X * this.m_ptpImpulse.y -
					r1Y * this.m_ptpImpulse.x +
					this.m_motorImpulse +
					this.m_limitImpulse);
			...
			b2.m_angularVelocity +=
				invI2 *
				(r2X * this.m_ptpImpulse.y -
					r2Y * this.m_ptpImpulse.x +
					this.m_motorImpulse +
					this.m_limitImpulse);
		} else {
	...
	SolveVelocityConstraints: function (step) {
		...
		var ptpCdotX =
			b2.m_linearVelocity.x +
			-b2.m_angularVelocity * r2Y -
			b1.m_linearVelocity.x -
			-b1.m_angularVelocity * r1Y;
		var ptpCdotY =
			b2.m_linearVelocity.y +
			b2.m_angularVelocity * r2X -
			b1.m_linearVelocity.y -
			b1.m_angularVelocity * r1X;
		var ptpImpulseX = -(
			this.m_ptpMass.col1.x * ptpCdotX +
			this.m_ptpMass.col2.x * ptpCdotY
		);
		var ptpImpulseY = -(
			this.m_ptpMass.col1.y * ptpCdotX +
			this.m_ptpMass.col2.y * ptpCdotY
		);
		...
		if (this.m_enableMotor && this.m_limitState != b2Joint.e_equalLimits) {
			var motorCdot =
				b2.m_angularVelocity - b1.m_angularVelocity - this.m_motorSpeed;
			var motorImpulse = -this.m_motorMass * motorCdot;
			var oldMotorImpulse = this.m_motorImpulse;
			this.m_motorImpulse = b2Math.b2Clamp(
				this.m_motorImpulse + motorImpulse,
				-step.dt * this.m_maxMotorTorque,
				step.dt * this.m_maxMotorTorque
			);
			motorImpulse = this.m_motorImpulse - oldMotorImpulse;
	...
	SolvePositionConstraints: function () {
		...
		if (this.m_enableLimit && this.m_limitState != b2Joint.e_inactiveLimit) {
			...
			if (this.m_limitState == b2Joint.e_equalLimits) {
				...
				limitC = b2Math.b2Clamp(
					limitC + b2Settings.b2_angularSlop,
					-b2Settings.b2_maxAngularCorrection,
					0.0
				);
				limitImpulse = -this.m_motorMass * limitC;
